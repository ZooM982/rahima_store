...
const MobileBottomNav = () => {
    ...
    return (
      <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-black backdrop-blur-xl border-t border-white/10 px-4 py-3 z-50 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.05)] rounded-t-[30px]">
        {adminLinks.map((link) => {
          ...
          return (
            <Link key={link.path} to={link.path} className={`flex flex-col items-center gap-1.5 p-2 min-w-[64px] transition-all ${active ? 'text-primary scale-110' : 'text-gray-400'}`}>
  ...
  return (
    <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white/80 backdrop-blur-xl border-t border-gray-100 px-6 py-3 pb-5 z-50 flex justify-between items-center shadow-[0_-10px_30px_rgba(0,0,0,0.05)] rounded-t-[30px]">
      <Link to="/" className={`flex flex-col items-center gap-1.5 p-2 min-w-[64px] transition-all ${isActive('/') ? 'text-primary scale-110' : 'text-gray-400'}`}>
        <Home size={22} strokeWidth={isActive('/') ? 2.5 : 2} />
        <span className="text-[10px] font-bold uppercase tracking-widest">Accueil</span>
      </Link>

      <Link to="/products" className={`flex flex-col items-center gap-1.5 p-2 min-w-[64px] transition-all ${isActive('/products') ? 'text-primary scale-110' : 'text-gray-400'}`}>
        <Search size={22} strokeWidth={isActive('/products') ? 2.5 : 2} />
        <span className="text-[10px] font-bold uppercase tracking-widest">Boutique</span>
      </Link>

      <Link to="/cart" className={`flex flex-col items-center gap-1.5 p-2 min-w-[64px] relative transition-all ${isActive('/cart') ? 'text-primary scale-110' : 'text-gray-400'}`}>
        <ShoppingBag size={22} strokeWidth={isActive('/cart') ? 2.5 : 2} />
        {cartCount > 0 && (
          <span className="absolute top-1 right-2 bg-primary text-white text-[8px] w-4 h-4 rounded-full flex items-center justify-center font-bold animate-bounce">
            {cartCount}
          </span>
        )}
      ...
      {isAdmin ? (
        ...
        <Link to={user ? "/profile" : "/login"} className={`flex flex-col items-center gap-1.5 p-2 min-w-[64px] transition-all ${isActive('/login') || isActive('/profile') ? 'text-primary scale-110' : 'text-gray-400'}`}>
          <User size={22} strokeWidth={isActive('/login') || isActive('/profile') ? 2.5 : 2} />
